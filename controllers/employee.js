        ...
        orm.all(`employees`, cb => {
            inquirer.prompt({
                ...
                orm.find("employees", "employeeid", eeid, cb => {
                    const { first_name, last_name, is_manager, roleid, managerid } = cb[0];
                    const eeChoices = [
                    ...
                    inquirer.prompt(
                        ...
                        if (updateArr[0] === "First name" || updateArr[0] === "Last name") {
                            const ugly_val = updateArr[0].toLowerCase().split(" ").join("_");
                            inquirer.prompt(
                                {
                                    type: 'input',
                                    ...
                                    const { new_name } = res;
                                    orm.update("employees", `${ugly_val}="${new_name}"`, `employeeid=${eeid}`, cb => {
                            ...
                            inquirer.prompt(
                                {
                                    ...
                                    orm.update("employees", `${ugly_val}=${is_manager}`, `employeeid=${eeid}`, cb => {
                                        console.log(`=============== Employee Manager Status Updated ============`);
                                        reinit();
                                    });
                                });
                        }
                                ...
                                inquirer.prompt(
                                    {
                                        ...
                                        role.split(":");
                                        const roleInput = role[0];
                                        orm.update("employees", `roleid=${roleInput}`, `employeeid=${eeid}`, cb => {
                            ...
                            orm.find("employees", "is_manager", true, cb => {
                                inquirer.prompt({
                                    ...
                                    manager.split(":");
                                    const mgrInput = manager[0];
                                    orm.update("employees", `managerid=${mgrInput}`, `employeeid=${eeid}`, cb => {
            ...
            inquirer.prompt({
                ...
                orm.find('employees', 'employeeid', eeArr[0], cb => {
                    ...
                    if (cb[0].managerid !== null) {
                        orm.someWHERE('employees', ['first_name', 'last_name'], 'employeeid', cb[0].managerid, mgrcb => {
                            console.log(`Reports to: ${mgrcb.first_name} ${mgrcb.last_name}`);
                        })  
                    }
                    else {
                    ...
                    if (cb[0].is_manager){
                        orm.someWHERE('employees', ['employeeid', 'first_name', 'last_name'], 'managerid', cb[0].employeeid, eecb => {
