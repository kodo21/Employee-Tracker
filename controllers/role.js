...
const role = {
        ...
        orm.some("departments", ["departmentid", "dept_name"], cb => {
            ...
            inquirer.prompt([
                ...
                },
                {
                    ...
                    name: "dept",
                    choices: () => {
                        let dlist = cb.map(obj => `${obj.departmentid}: ${obj.dept_name}`);
                        return dlist;
                    }
                }
            ]).then(answers => {
                ...
                orm.create("roles", ["title", "salary", "departmentid"], [role_title, salary, deptInput], cb => {
                            ...
                            inquirer.prompt(
                                {
                                    ...
                                    const { new_val } = res;
                                    orm.update("roles", `${ugly_val} = "${new_val}"`, `roleid = ${roleid}`, cb => {
                                        console.log(`=============== Role Title or Salary Updated ============`);
                                        reinit();
                                    });
                                })
                        }
                                ...
                                inquirer.prompt(
                                    {
                                        ...
                                        name: "dept",
                                        choices: () => {
                                            let rlist = cb.map(obj => `${obj.departmentid}: ${obj.dept_name}`);
                                            return rlist;
                                        }
                                    }).then(answers => {
                                        const { dept } = answers;
                                        dept.split(":");
                                        const deptInput = dept[0];
                                        orm.update("roles", `departmentid = ${deptInput}`, `roleid = ${roleid}`, cb => {
                                            console.log(`=============== Role Department Updated ============`);
                                            reinit();
                                        });
                                    })
                            })
    ...
    allEmployees: reinit => {
        //show all employees in a selected role
        orm.all(`roles`, cb => {
            inquirer.prompt({
                type: 'list',
