...
const department = {
                ...
                inquirer.prompt(
                    {
                        ...
                        const { new_val } = res;
                        orm.update("departments", `dept_name = "${new_val}"`, `departmentid = ${deptid}`, cb => {
            ...
            inquirer.prompt({
                ...
                orm.someWHERE('roles', ['roleid'], 'departmentid', deptArr[0], cb => {
                    ...
                    orm.findOR('employees', "roleid", roleids, cb => {
                        console.log(`=============== All employees in ${deptArr[1]} ============`);
                        console.table(cb);
                        reinit();
                    })
    ...
    allRoles: reinit => {
        //show all roles in a selected department
        orm.all(`departments`, cb => {
            inquirer.prompt({
                type: 'list',
                message: `Which department's employees would you like to view?`,
                ...
                orm.find('roles', 'departmentid', deptArr[0], cb => {
                    console.log(`=============== All roles in${deptArr[1]} ============`);
                    console.table(cb);
                    reinit();
                })
        ...
        orm.all(`departments`, cb => {
            inquirer.prompt({
                ...
                orm.salaries(deptArr[0], cb => {
                    console.log(`Total budget for${deptArr[1]}: $${cb[0].total_salaries}`)
                    reinit();
                })
